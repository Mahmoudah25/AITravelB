...
namespace AITravelB.Infrastructure.ExteranlService.Groq
{
    public class GroqItineraryService : IItineraryAiService
    {
        ...
        public async Task<ItineraryResult> GenerateItineraryAsync(string destination, int days, decimal budget)
        {
            var apiKey = configuration["ExternalServices:AiProvider:ApiKey"]
               ?? throw new InvalidOperationException("API key is missing.");
            var prompt = GenerateGroqQuery(destination, days, budget);
        ...
        private string GenerateGroqQuery(string destination, int days, decimal budget)
        {
